...
class ManaCheckModule : public IModule {
public:
...
class SimpleDamageModule : public IModule {
public:
...
class BurnConditionModule : public IModule {
public:
...
class OutputModule : public IModule {
public:
...
TEST_CASE("Single module is executed") {
    auto engine = Chained::withModules(
        ManaCheckModule(),
        SimpleDamageModule(),
        BurnConditionModule(),
        OutputModule()
    ...
    auto outcome = engine.process(ctx);

    REQUIRE(outcome.damageDealt == 40);
    REQUIRE(outcome.type == "fire");
    REQUIRE(outcome.flags.size() == 1);
}
